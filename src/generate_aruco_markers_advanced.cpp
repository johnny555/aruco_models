/**
 * @file generate_aruco_markers_advanced.cpp
 * @brief Advanced C++ ArUco marker generator with multiple dictionary support
 * 
...
class AdvancedArucoMarkerGenerator {
private:
    int marker_size_;
...
public:
     ...
     */
    AdvancedArucoMarkerGenerator(int marker_size = 200, const std::string& output_dir = "../textures")
        : marker_size_(marker_size), output_dir_(output_dir) {
        
        // Create output directory if it doesn't exist
        fs::create_directories(output_dir_);
        
        std::cout << "Advanced ArUco Marker Generator initialized:" << std::endl;
...
AdvancedArucoMarkerGenerator::dictionary_map_ = {
...
AdvancedArucoMarkerGenerator::dictionary_sizes_ = {
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "--help" || arg == "-h") {
            AdvancedArucoMarkerGenerator::printUsage(argv[0]);
            return 0;
        } else if (arg == "--list-dicts") {
            AdvancedArucoMarkerGenerator::listDictionaries();
            return 0;
        } else if (arg == "--info" && i + 1 < argc) {
            AdvancedArucoMarkerGenerator::printDictionaryInfo(argv[++i]);
            return 0;
    ...
    try {
        // Validate dictionary
        if (AdvancedArucoMarkerGenerator::getDictionarySize(dict_name) == 0) {
            std::cerr << "Error: Unknown dictionary '" << dict_name << "'" << std::endl;
        ...
        // Create generator
        AdvancedArucoMarkerGenerator generator(marker_size, output_dir);
        
        // Print dictionary info
        std::cout << "\nUsing dictionary: " << dict_name << std::endl;
        AdvancedArucoMarkerGenerator::printDictionaryInfo(dict_name);
