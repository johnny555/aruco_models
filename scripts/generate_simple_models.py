#!/usr/bin/env python3
"""
Alternative ArUco model generator using simple diffuse textures instead of PBR.
This should work with both classic Gazebo and Ignition Gazebo.
"""
...
def create_model_sdf_simple(marker_id, tag_size=0.14, thickness=0.001):
    """Create SDF file for an ArUco marker model with simple texture."""
    return f"""<?xml version="1.0" ?>
<sdf version="1.7">
    ...
    <link name="link">
      ...
      <visual name="visual">
        ...
        <material>
          ...
          <texture>model://aruco-{marker_id}/materials/textures/aruco_{marker_id:03d}.png</texture>
        </material>
      </visual>
...
def create_aruco_model_simple(marker_id, models_dir, textures_dir, tag_size=0.14):
    """Create a complete ArUco marker model with simple texture."""
    ...
    with open(sdf_path, "w") as f:
        f.write(create_model_sdf_simple(marker_id, tag_size))
    ...
    if os.path.exists(texture_src):
        ...
        print(f"Created simple texture model: {model_name}")
        return True
    else:
...
def main():
    parser = argparse.ArgumentParser(description='Generate Gazebo ArUco marker models with simple textures')
    parser.add_argument('--models-dir', default='models_simple', 
    ...
    print(f"Generating simple texture Gazebo models for ArUco markers: {marker_ids}")
    ...
    for marker_id in marker_ids:
        if create_aruco_model_simple(marker_id, args.models_dir, args.textures_dir, args.tag_size):
            success_count += 1
    
    print(f"Successfully created {success_count} simple texture models out of {len(marker_ids)} requested.")
