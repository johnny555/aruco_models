...
def generate_launch_description():
    ...
    # Start a simulation with the ArUco test world
    aruco_world_uri = join(get_package_share_directory("aruco_models"), "aruco_test_improved.world")
    path = join(get_package_share_directory("ros_gz_sim"), "launch", "gz_sim.launch.py")
    
    gazebo_sim = IncludeLaunchDescription(path,
                                          launch_arguments=[("gz_args", '-r '+ aruco_world_uri)])
    ...
    robot_file = join(get_package_share_directory("aruco_models"), "aruco_test_robot.urdf.xacro")
    ...
    static_pub = Node(package="tf2_ros", 
                      ...
                      arguments=["0","0","0","0","0","0", "lidar_2d_link", "aruco_test_robot/base_footprint/lidar_2d_v1", ])
